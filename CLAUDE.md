...
- **Claude가 하는 모든 작업은 예외 없이 `Staging` 브랜치에 먼저 커밋·푸시하여 스테이징에 빌드한다.** 코드·컨텐츠·문서·설정 등 종류를 불문한다.
- **main 반영(프로덕션 빌드·배포)은 사용자가 스테이징(https://letspilatesla.com/staging/)에서 확인하고 명시적으로 허락한 뒤에만 진행한다.**
- Claude는 사용자 허락 없이 `Staging` → `main` PR 생성·병합·배포를 **절대 하지 않는다**.
